...
class WaterUnmark():
    ...
    def estimate_watermark(self):
        ...
        for img in self._raw_imgs:
            ...
            elementwise_2Dmag_fn = lambda n, Gx, Gy: np.vectorize(lambda x, y: (x ** 2 + y ** 2) ** 0.5)(Gx[:, :, n],
                                                                                                         Gy[:, :,
        ...
        for _ in range(1):
            ...
            # extract edges
            img_edges_median = apply_fn_to_all_colorchannels(edge_detect_fn, 3, 1, 0.3, img_Gmag_median).astype(
                bool)  # run canny edge detection
        ...
        w_estimate = apply_fn_to_all_colorchannels(reconstruct_w_fn, 3, img_Gx_reconstruct, img_Gy_reconstruct,
                                                   img_boundary_median)
...
img_dir_filepath = os.path.join(os.path.dirname(os.path.abspath(__file__), r"examples/123RF")
